...
class MBTI extends Component {
  ...
  handleNext = () => {
    ...
    // 切换下一题
    if(this.state.cur_index < this.state.total-1){
    } else {
      Toast.info('没有下一题哦！', 1)
    }
  }
  ...
  render() {
    const { cur_index, questions, cur_select, categoryName, total } = this.state;
    ...
    return (
      <div style={{backgroundColor: '#fff', height: '100%', paddingTop: '20px'}}>
        ...
        <List renderHeader={() => <div><span>选择答案</span>  <span style={{float: 'right'}}>进度 {cur_index+1}/{total}</span></div>}>
